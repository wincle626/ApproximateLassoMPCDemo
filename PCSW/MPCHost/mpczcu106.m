...
%     gamma = 0.1*gamma_max;
    ...
    for i = 1:size(uMPC_pgd,1)
        plot(t_unconstrained(1:ct),max(min(uMPC_pgd(i,1:ct), ...
    ...
    hold on;
    for i = 5:7
        plot(t_unconstrained(1:ct),max(min(yMPC_pgd(i,1:ct), ...
    ...
    subplot(4,1,3);
    hold on;
    plot(t_unconstrained(1:ct),max(min(yMPC_pgd(4,1:ct), ...
...
%     mpc3dgui([yMPC_pgd(7,ct) yMPC_pgd(6,ct) yMPC_pgd(5,ct)], h2);
    ...
    rotateObj(ncomp, s, ... 
        h17, h18, h19, ...
        yMPC_pgd(7,ct), yMPC_pgd(6,ct), yMPC_pgd(5,ct));
%         h11, h12, h13, ...
%         h14, h15, h16, ...
%     pause(0.5)
end
